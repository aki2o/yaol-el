    ...
    (-map (lambda (pos)
            ...
            (if (re-search-backward (rx-to-string `(or bol ,@end-of-expressions)) nil t)
                `(:type ,(plist-get pos :type)
                        :string ,(concat (buffer-substring-no-properties (match-end 0) (plist-get pos :point)) (plist-get pos :string))
                        :point ,(match-end 0))
              pos))
          positions)))
...
(cl-defun yaol-expand-head-position (positions &key (end-of-headers '()) (include-indent t))
  (-map (lambda (pos)
          (let ((pt (progn
            ...
            `(:type ,(plist-get pos :type)
                    :string ,(buffer-substring-no-properties pt (point))
                    :point ,pt)))
        positions))
...
(defun yaol-new-nodes-by-positions (positions)
  (remove nil
          (cl-loop with open-positions = nil
                   with positions = (-sort (lambda (a b) (< (plist-get a :point) (plist-get b :point)))
                                           positions)
                   while positions
...
(defun yaol-new-nodes-by-indent ()
  (cl-labels ((get-current-level (string)
                                 (if (string-match (rx bos (group (+ (any " \t"))) (not space)) string)
...
(defun yaol-show-region (beg end)
  (remove-overlays beg end 'invisible 'yaol))
...
(cl-defun yaol-handle-visibility (node &key head body (child-head 0) (child-body 0))
  (cl-labels ((shown-child? (v s)
              ...
              (show-head? ()
                          (if (stringp head)
                              (string-match head (buffer-string (yaol-node-beg node) (yaol-node-fold-beg node)))
                            head))
              (show-body? ()
                          (if (stringp body)
                              (string-match body (buffer-string (yaol-node-fold-beg node) (yaol-node-fold-end node)))))
    ...
    (let ((show-children? (-any? (lambda (c)
                                   (or (shown-child? child-head (buffer-string (yaol-node-beg c) (yaol-node-fold-beg c)))
                                       (shown-child? child-body (buffer-string (yaol-node-fold-beg c) (yaol-node-fold-end c)))))
...
(defun yaol-hide-current-level-parts (beg end children)
  (let* ((covered-children (-filter (lambda (c)
    ...
    (cl-loop with prev-child = nil
             ...
             if (not overlap?)
             do (progn
                  (yaol-create-overlay next-beg (yaol-node-beg child) :replacement replacement)
    ...
    (yaol-create-overlay next-beg end :replacement replacement)))

(cl-defun yaol-create-overlay (beg end &key (buffer (current-buffer)) replacement)
  (when (< beg end)
    (let ((ov (make-overlay beg end buffer)))
      (overlay-put ov 'invisible 'yaol)
      ...
      (when replacement
        (overlay-put ov 'display yaol-fold-replacement)
        (overlay-put ov 'face 'yaol-fold-replacement-face))
      ov)))
